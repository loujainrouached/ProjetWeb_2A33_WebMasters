...
public function afficheNomP($ID_pays) {
    try {
        $pdo = config::getConnexion();
        $query = $pdo->prepare("SELECT guides.*, NomP AS NomP FROM guides INNER JOIN pays ON guides.ID_pays = pays.ID_pays WHERE guides.ID_pays = :ID");
        $query->execute(["ID" => $ID_pays]);
        return $query->fetchAll();
    } catch (PDOException $e) {
        echo $e->getMessage();
